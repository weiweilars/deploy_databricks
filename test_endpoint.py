...
class _DatabricksClientBase(BaseModel, ABC):
    ...
    def post_raw(self, request: Any) -> Any:
        ...
        request = json.dumps(request, allow_nan=True)
        
        response = requests.request(method='POST', headers=headers, url=self.api_url, data=request)
...
class DatabricksPyfunc(LLM):
    ...
    * an automatically generated temporary token if running inside a Databricks
    ...
    def transform_input_fn(**kwargs):
        result={'dataframe_split': {"index": [0], "columns": ["prompt"], 'data': []}, 'params': {}}
        result['dataframe_split']["data"].append([kwargs['prompt']])
        kwargs.pop('prompt')
...
with mlflow.start_run() as run:
    ...
    llm = DatabricksPyfunc(host='enb-capgemini.cloud.databricks.com',endpoint_name="llama_pyfunc_3",model_kwargs={"max_tokens":10})
    mlflow.log_param("test", llm(prompt=""" Give only funny answers: What is life?"""))
